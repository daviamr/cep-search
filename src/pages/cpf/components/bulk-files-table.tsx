...
import type { CpfBulkFile } from "@/lib/api/cpf"
import {
  formatFileSize,
  formatNumber,
  formatDateTime,
...
type BulkFilesTableProps = {
  files: CpfBulkFile[]
  isLoading?: boolean
  onDownload: (file: CpfBulkFile) => void | Promise<void>
...
export function BulkFilesTable({
  ...
  const [fileToDelete, setFileToDelete] = useState<CpfBulkFile | null>(null)
  ...
  if (files.length === 0) {
    return (
      <QueryEmptyState
        ...
        description="Envie uma planilha com CPFs para começar o processamento em massa."
      />
    )
  }
